...
* stores the address book data i.e., all `Person` objects (which are contained in a `UniquePersonList` object where only duplicated person names and phone are not allowed).
