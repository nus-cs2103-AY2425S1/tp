...
4. **Improve error messages:** Currently, the error messages inform users that a command is incorrect. We plan to improve the error messages to provide more specific feedback to users on what went wrong. For example, if a user has missing/extra fields in the `add` command, the error message will specify which fields are causing the error.
