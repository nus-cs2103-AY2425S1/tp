...
2. **Add support for 's/o' in name field:** The current implementation only allows alphanumeric characters, spaces, and dashes, and so users have to work around this restriction by using "s o" or "son of". In future, we will allow the special string "s/o" to accomodate for this.
