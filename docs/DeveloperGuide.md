...
4. **Maintain the state of the person list after modification:** If a user used the `find` command to filter the person list before modifying the list (`add`, `edit`, etc.), the person list will reset to show all entries. We plan to maintain the current filtered person list and retain list position after modifications. Also, any added elderly will be appended to the back of the filtered list to retain list position.
