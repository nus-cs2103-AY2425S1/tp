...
This command aids in the creation of an `Appointment` for a given `Person` in the currently loaded `Model`. Implementation of this command touches many aspects of the application including UI and `Model`.

#### Overview
The `ApponintmentListPanel` will consequently update with a new `AppointmentCard` as the `Model` supplies a new `OwnedAppointment` record that associates the new `Appointment` and the target `Person`.  

`addAppt` is the command word of `AddApptCommand`. `AddApptCommand` extends the `Command` abstract class and is generated by `AddApptCommandParser#parse(String args)` after being called by `AddressBookParser#parseCommand(String UserInput)`. This call chain is initiated by `LogicManager#execute(String commandText)`, which then takes the returned `AddApptCommand` and calls `AddApptCommand#execute(Model model)`, passing in a reference of the current `Model`. The execution will create the new `Appointment`, and updates the received `Model`. 

`AppointmentListPanel` is a wrapper for a JavaFX `ListView` that utilises an `ObservableList` that monitors a container of `OwnedAppointment` elements for any changes. The reference to this `OwnedAppointment` is provided by `Logic#getFilteredAppointmentList()` during construction. The `ListView`'s cell factory method is a lambda function that constructs a new `AppointmentListPanel#AppointmentListViewCell` object, which occurs whenever a change is observed in the `ObservableList`, updating the UI. 

`Logic#getFilteredAppointmentList()` returns the `FilteredList` from `Model`, which receives this list from the assoicated `AddressBook`. During the execution stage of `AddApptCommand`, the target `Person` is removed from `AddressBook` by `ModelManager` and then a new `Person` with the new `Appointment` is added into the `AddressBook` using `ModelManager#setPerson(Perosn person)`. This method calls `AddressBook#setPerson(Person person)`, which adds the new person and also calls `AddressBook#updateAppointmentList()`, which clears the `AddressBook#appointments` list and repopulates it with new `OwnedAppointments` for each `Appointment` in each `Person` in the `AddressBook#Persons` list. Due to the event listener of `AppointmentListPanel`, this change triggers a redrawing of the `ListView` in the UI. 
