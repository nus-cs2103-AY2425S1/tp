...
3. **Allow modification to elderly in the history view:** 
Currently, users are unable to perform any modification to elderly in the history view as it is meant for viewing purposes only. We plan to allow users to make modifications to the elderly whose details are specified in the history view such as making changes to the elderly details and marking the elderly as called, etc.
4. **Maintain the current filtered person list and list position after modification of the elderly list:** If a user used the `find` command to filter the person list before modifying the list (`add`, `edit`, etc.), the person list will reset to show all entries. We plan to maintain the current filtered person list and retain list position after modifications. Also, any added elderly will be appended to the back of the filtered list to retain list position.
