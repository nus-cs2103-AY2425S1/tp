...
4. **Improve error messages:** Currently, the error messages are generic and only state that a command is incorrect. We plan to improve the error messages to provide more specific feedback to users on what went wrong. For example, if a user has missing fields in the `add` command, the error message will specify which fields are missing.
5. **Improve error message ordering:** When in the history view, if a user enters an invalid command, the invalid command error message is shown. This is still the case if the command is disabled in the history view. We plan to change this behaviour such that the disabled command error message is shown first before the invalid command error message.
