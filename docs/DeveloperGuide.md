...
**Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the ``, which `Member` references. This allows `` to only require one `Tag` object per unique tag, instead of each `Member` needing their own `Tag` objects.<br>
