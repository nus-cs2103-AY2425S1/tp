...
2. **Include validation for telegram ID:** Currently, KonTActs does not chack for the telegram ID's length despite it not being a valid username in telegram. In future updates, we are planning to provide proper telegram ID validation to ensure that there can not be two people with the same telegram IDs.
3. **Add flexibility to attendance:** Currently, the maximum number of weeks that can be inputted is 13 since there are 13 weeks in a semester. However, it is possible that the user would want to use it for a different amount of weeks such as during special terms. Thus, we plan on incorporating comamnds to edit the start or end week of the attendance.
