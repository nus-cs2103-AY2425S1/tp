...
7. **Find Command Does Not Persist Filtered State for Persons**: Currently, executing commands such as `add` or `edit` resets the person filter, causing the UI to refresh. This can be inconvenient for users wishing to add new entries to an already filtered list of persons. Future enhancements aim to retain the person filter state for such commands.
