...
|                         | `FIELD\NEW_VALUE` | `n\ `, `i\ `, `yg\ `, `p\ `, `e\ `, `a\ ` or `s\ ` | Field to be edited (e.g., Name(n), NRIC(i), Year Group(yg), Phone(p), Email(e), Address(a), Subject(s)) and new value for the specified field. | FIELD must be the one of the specified fields and NEW_VALUE must follow the constraints of the specified field.                                                 |
