...
1. **Rounding up to 2 decimal places** in the `pay` and `owe` commands can result in a minor difference of tuition fee based on the number of hours one session lasts, e.g. a student with the rate of $100.01/hr pays $300.03 for a 3-hour lesson, but pays $300.04 in total for two 1.5-hour lessons. However, this issue is less likely to impact tutors, as their hourly rates typically do not extend to two decimal places. Therefore, in most cases, the differences would be negligible. 
--------------------------------------------------------------------------------------------------------------------
