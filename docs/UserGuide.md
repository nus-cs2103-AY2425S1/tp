...
* Guests with the exact same name cannot be added to t---
  layout: page
  title: User Guide
---
...
* Only leading or trailing spaces in any tag or guest name will be ignored, for increased flexibility.
  e.g. `bride's side` and `bride's  side` are considered different tags.
  e.g. `John Doe` and `John  Doe` are considered different guests.
  e.g. `  bride's side` and `bride's side` are considered the same tag.
...
he guest list. Users are advised to add their guests' last names or other identifiers if needed.
* Phone numbers must be **exactly** 8 digits.
* Tags must be created before they can be assigned to a guest. Refer to [newtag](#creating-a-new-tag-newtag) on how to create a tag.
<div markdown="span" class="alert alert-primary">:bulb: **Tip:**
A guest can have any number of tags (including 0).
