...
* `CLAIM_AMOUNT_IN_DOLLARS_AND_CENTS` is the claim amount written in `DD.CC` format eg. `151.10`. So it can contain any number of digits for dollars but must be exactly 2 digits for cents, which is to ensure that the cents has been correctly indicated.
