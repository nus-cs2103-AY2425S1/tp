...
3. `CLAIM_ID` : a unique id for each claim generated by an external organisation before the claim is added to this app.
4. `CLAIM_AMOUNT_IN_DOLLARS_AND_CENTS` is the claim amount written in `DD.CC` format eg. `151.10`. So, it can contain
any number of digits for dollars but must be exactly 2 digits for cents. The claim amount supported is only up till 1 million.
