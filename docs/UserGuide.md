...
InternBuddy is a **desktop app** designed for **university students** navigating the intensive process of managing numerous internship applications and contacts. InternBuddy is an **AddressBook** optimized for use via a **Command Line Interface** (CLI) while still having the benefits of a **Graphical User Interface** (GUI). For fast typers, InternBuddy streamlines contact management tasks, making it faster and more effective than traditional GUI-only applications.
