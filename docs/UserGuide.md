...
Format: `deleteInsurance <client_id> iid/ <insurance_id>`
...
Examples:
* `deleteInsurance 1 iid/ 1` deletes the insurance plan of id 1 from the client with id 1

### Adding a claim to the client : `addClaim`

Adds a claim to the insurance plan of a client.

Format: `addClaim <client_id> iid/ <insurance_id> cid/ <claim_id> ca/ <claim_amount_in_dollars_and_cents>`

* `CLIENT_ID` and `INSURANCE_ID` are found in the address book matching the client and their associated insurance plan
...
* `CLAIM_AMOUNT_IN_DOLLARS` is the claim amount written in `DOLLARS.CENTS` format eg. `15.10`
...
Deletes a claim from the insurance plan of a client.

Format: `deleteClaim <client_id> iid/ <insurance_id> cid/ <claim_id>`
