...
const VariableRenderer_1 = require("./VariableRenderer");
...
class VariableProcessor {
    ...
    baseUrlMap) {
        ...
        this.baseUrlMap.forEach((siteRootPath) => {
            ...
            this.variableRendererMap[siteRootPath] = new VariableRenderer_1.VariableRenderer(siteRootPath);
        });
    }
    /*
     * --------------------------------------------------
    ...
    renderAndAddUserDefinedVariable(site, name, value) {
        if (name === undefined) {
            logger.warn(`Variable with no name detected! It will be ignored. Value: ${value}. Site: ${site}.`);
            logger.warn('Check _markbind/variables.md for variable declarations.');
            return;
        }
        const renderedVal = this.variableRendererMap[site].renderString(value, this.userDefinedVariablesMap[site], new PageSources_1.PageSources());
    ...
    getParentSiteVariables(contentFilePath) {
        const parentSitePath = urlUtil.getParentSiteAbsolutePath(contentFilePath, this.rootPath, this.baseUrlMap);
    ...
    renderWithSiteVariables(contentFilePath, pageSources, lowerPriorityVariables = {}) {
        ...
        const parentSitePath = urlUtil.getParentSiteAbsolutePath(contentFilePath, this.rootPath, this.baseUrlMap);
        return this.variableRendererMap[parentSitePath].renderFile(contentFilePath, Object.assign(Object.assign({}, lowerPriorityVariables), userDefinedVariables), pageSources);
    }
    /*
     * --------------------------------------------------
    ...
    static extractIncludeChildElementVariables(includeElement) {
        ...
        includeElement.children.forEach((child) => {
            ...
            const variableName = ((_a = child.attribs) === null || _a === void 0 ? void 0 : _a.name) || ((_b = child.attribs) === null || _b === void 0 ? void 0 : _b.id);
            if (!variableName) {
                logger.warn(`Missing 'name' or 'id' in variable for ${includeElement.attribs.src}'s include in ${includeElement.attribs[constants_1.ATTRIB_CWF]}.\n`);
                return;
            }
            if (!includeChildVariables[variableName]) {
                includeChildVariables[variableName] = (0, cheerio_1.default)(child).html() || '';
            }
        });
    ...
    static extractIncludeVariables(includeElement) {
        ...
        return Object.assign(Object.assign({}, includeChildVariables), includeInlineVariables);
    }
    /*
     * --------------------------------------------------
    ...
    renderIncludeFile(filePath, pageSources, node, context, asIfAt) {
        ...
        const variables = Object.assign(Object.assign({}, includeVariables), context.variables);
