...
function createMinimalForm(pluginContext) {
    const $replaceNode = (0, cheerio_1.default)('<form onsubmit="submitForm(this)"></form>');
    $replaceNode.append(`<input type="hidden" name="access_key" value=${pluginContext.accessKey}>`);
    return $replaceNode;
}
function deleteWeb3FormAttributes(node) {
    delete node.attribs.default;
    delete node.attribs.header;
}
function transformFormInputs(child) {
    if (child.type !== 'tag') {
        return;
    }
    const $node = (0, cheerio_1.default)(child);
    switch (child.name) {
        case 'name-input':
...
function generateFormContainer(node) {
    if (node.type !== 'tag') {
        return;
    }
    node.name = 'box';
    const $node = (0, cheerio_1.default)(node);
    ...
    if (_.has(node.attribs, 'header')) {
        ...
        const $headerNode = (0, cheerio_1.default)(header);
        $node.prepend($headerNode);
    }
}
function createCustomForm(pluginContext, node) {
    const $node = (0, cheerio_1.default)(node);
...
function isDefaultContactForm(node) {
    return _.has(node.attribs, 'default');
}
function createDefaultContactForm(pluginContext, node) {
    const $node = (0, cheerio_1.default)(node);
    if (!_.has(node.attribs, 'header')) {
