...
<head>
    <title>Code coverage report for src/utils/urlUtil.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/utils</a> urlUtil.ts</h1>
        <div class='clearfix'>
...
<a name='L76'></a><a href='#L76'>76</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">23x</span>
...
function getParentSiteAbsolutePath(filePath: string, root: string, lookUp: Set&lt;string&gt;) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!path.isAbsolute(filePath)) {
<span class="cstat-no" title="statement not covered" >    throw new Error(`Non-absolute path given to getParentSiteAbsolutePath: '${filePath}'`);</span>
  }
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!pathIsInside(filePath, root)) {
<span class="cstat-no" title="statement not covered" >    throw new Error(`Path given '${filePath}' is not in root '${root}'`);</span>
  }
&nbsp;
  function calculate(file: string): string {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (file === root) {
...
&nbsp;
    ...
    return lookUp.has(parent)
      ? parent
      : <span class="branch-1 cbranch-no" title="branch not covered" >calculate(parent);</span>
  }
&nbsp;
 ...
 */
function <span class="fstat-no" title="function not covered" >getParentSiteAbsoluteAndRelativePaths(</span>filePath: string, root: string, lookUp: Set&lt;string&gt;) {
  const absolute = <span class="cstat-no" title="statement not covered" >getParentSiteAbsolutePath(filePath, root, lookUp);</span>
 ...
 */
function <span class="fstat-no" title="function not covered" >calculateBoilerplateFilePath(</span>
  pathInBoilerplates: string, asIfAt: string, config: Record&lt;string, any&gt;,
) {
<span class="cstat-no" title="statement not covered" >  return path.resolve(getParentSiteAbsolutePath(asIfAt, config.rootPath, config.baseUrlMap),</span>
...
function stripBaseUrl(src: string, baseUrl: string) {
  return src.startsWith(baseUrl)
    ...
    : <span class="branch-1 cbranch-no" title="branch not covered" >src;</span>
}
&nbsp;
export {
