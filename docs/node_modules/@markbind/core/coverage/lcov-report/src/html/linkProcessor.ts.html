...
<head>
    <title>Code coverage report for src/html/linkProcessor.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/html</a> linkProcessor.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">84.55% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>104/123</span>
            </div>
...
<a name='L277'></a><a href='#L277'>277</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">13x</span>
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (path.isAbsolute(resourcePath)) {
    // Do not rewrite.
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (node.name in pluginTagConfig &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >pluginTagConfig[node.name].attributes)</span> {
<span class="cstat-no" title="statement not covered" >    pluginTagConfig[node.name].attributes.forEach(<span class="fstat-no" title="function not covered" >(a</span>ttrConfig) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      <span class="missing-if-branch" title="if path not taken" >I</span>if (attrConfig.isRelative &amp;&amp; node.attribs) {</span>
        const resourcePath = <span class="cstat-no" title="statement not covered" >node.attribs[attrConfig.name];</span>
<span class="cstat-no" title="statement not covered" >        _convertRelativeLink(node, cwf, rootPath, baseUrl, resourcePath, attrConfig.name);</span>
      }
    });
  }
}
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (urlUtil.isUrl(href)) {
      // Not intralink
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
...
&nbsp;
    ...
    const fragment = hrefUrl.hash === null ? <span class="branch-0 cbranch-no" title="branch not covered" >'' </span>: hrefUrl.hash;
    const pathName = hrefUrl.pathname === null ? <span class="branch-0 cbranch-no" title="branch not covered" >'' </span>: hrefUrl.pathname;
...
function isValidPageSource(resourcePath: string, config: NodeProcessorConfig) {
  const relativeResourcePath = resourcePath.startsWith('/')
    ...
    : <span class="branch-1 cbranch-no" title="branch not covered" >resourcePath;</span>
...
function isValidFileAsset(resourcePath: string, config: NodeProcessorConfig) {
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (relativeResourcePath &amp;&amp; fileIgnore.ignores(relativeResourcePath)) {
...
&nbsp;
  ...
  if (hasNoFileExtension) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (hash !== undefined
      &amp;&amp; (<span class="branch-1 cbranch-no" title="branch not covered" >!filePathToHashesMap.get(asFileAsset) </span>|| <span class="branch-2 cbranch-no" title="branch not covered" >!filePathToHashesMap.get(asFileAsset)!.has(hash))</span>) {
<span class="cstat-no" title="statement not covered" >      logger.warn(hashErr);</span>
<span class="cstat-no" title="statement not covered" >      return 'Intralink with no extension is a valid Page Source or File Asset but hash is not found';</span>
    }
...
export function collectSource(node: MbNode, rootPath: string,
  ...
  if (!tagConfig || <span class="branch-1 cbranch-no" title="branch not covered" >!tagConfig.attributes)</span> {
    return;
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  tagConfig.attributes.forEach(<span class="fstat-no" title="function not covered" >(a</span>ttrConfig) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (!attrConfig.isSourceFile) {</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    const sourceFileLink = <span class="cstat-no" title="statement not covered" >node.attribs[attrConfig.name];</span>
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (!sourceFileLink || urlUtil.isUrl(sourceFileLink)) {</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    const linkWithoutBaseUrl = <span class="cstat-no" title="statement not covered" >urlUtil.stripBaseUrl(sourceFileLink, baseUrl);</span>
    const linkWithoutLeadingSlash = <span class="cstat-no" title="statement not covered" >linkWithoutBaseUrl.startsWith('/')</span>
      ? linkWithoutBaseUrl.substring(1)
      : linkWithoutBaseUrl;
&nbsp;
    const fullResourcePath = <span class="cstat-no" title="statement not covered" >path.join(rootPath, linkWithoutLeadingSlash);</span>
<span class="cstat-no" title="statement not covered" >    pageSources.staticIncludeSrc.push({ to: fullResourcePath });</span>
  });
}
&nbsp;</pre></td></tr></table></pre>
