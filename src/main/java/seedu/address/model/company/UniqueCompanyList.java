 ...
 * allow nulls.
 * A company is considered unique by comparing using
 * {@code Company#isSamePerson(Company)}. As such, adding and updating of
 * persons uses Company#isSamePerson(Company) for equality so as to ensure that
 * the company being added or updated is
 * unique in terms of identity in the UniqueCompanyList. However, the removal of
 * a company uses Company#equals(Object) so
...
public class UniqueCompanyList implements Iterable<Company> {
    ...
    private final ObservableList<Company> internalUnmodifiableList = FXCollections
