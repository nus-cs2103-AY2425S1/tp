...
/**
 * A list of employees that enforces uniqueness between its elements and does
 * not allow nulls. an employee is considered unique by comparing using
 * {@code Employee#isSameEmployee()}. As such, adding and updating of
 * employees uses Employee#isSameEmployee(Employee) for equality so as to ensure
 * that the employee being added or updated is unique in terms of identity in
 * the UniqueEmployeeList. However, the removal of an employee uses
 * Employee#equals(Object) so as to ensure that the employee with exactly the
...
public class UniqueEmployeeList implements Iterable<Employee> {
    ...
    /**
     * Returns true if the list contains an equivalent employee as the given
     * argument.
     */
    ...
    /**
     * Returns true if the list contains an {@code Employee} with the given
     * {@code EmployeeId}.
     */
