...
/**
 * A list of weddings that enforces uniqueness and does not allow nulls.
 * A wedding is considered unique by comparing using {@code Wedding#isSameWedding(Wedding)}. As such, adding and
 * updating of tags uses Wedding#isSameWedding(Wedding) for equality to ensure that the wedding being added or updated
 * is unique in terms of identity in the UniqueWeddingList. However, the removal of a wedding uses
 * Wedding#equals(Object) to ensure that the wedding with exactly the same fields will be removed.
