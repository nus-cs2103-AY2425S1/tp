...
/**
 * Tests that a {@code Meetup}'s {@code Subject} range contains the keywords.
 */
public class MeetUpContainsKeywordsPredicate implements Predicate<MeetUp> {
    private final List<String> keywords;

    /**
     * Meetup subject must be present and not null.
     */
    ...
    public boolean test(MeetUp meetUp) {
        return keywords.stream()
                .anyMatch(keyword -> StringUtil.containsWordIgnoreCase(meetUp.getSubject().meetUpFullSubject, keyword));
    }
