...
class JsonAdaptedDoctor {
    ...
    public JsonAdaptedDoctor(Doctor source) {
        ...
        speciality = source.getSpeciality().value;
    ...
    public Doctor toModelType() throws IllegalValueException {
        ...
        if (speciality == null) {
            throw new IllegalValueException(String.format(MISSING_FIELD_MESSAGE_FORMAT, Speciality.class.getSimpleName()));
        }
        if (!Speciality.isValidSpeciality(speciality)) {
            throw new IllegalValueException(Address.MESSAGE_CONSTRAINTS);
        }
        final Speciality modelSpecialty = new Speciality(speciality);

        final Set<Tag> modelTags = new HashSet<>(personTags);
        return new Doctor(modelName, modelPhone, modelEmail, modelAddress, modelSpecialty, modelTags);
    }
