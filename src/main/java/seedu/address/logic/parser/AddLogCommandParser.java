...
public class AddLogCommandParser implements Parser<AddLogCommand> {
    ...
    public AddLogCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_IDENTITY_NUMBER, PREFIX_LOG, PREFIX_DATE);

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_IDENTITY_NUMBER, PREFIX_LOG, PREFIX_DATE);

        if (argMultimap.getValue(PREFIX_IDENTITY_NUMBER).isEmpty() || argMultimap.getValue(PREFIX_LOG).isEmpty() || argMultimap.getValue(PREFIX_DATE).isEmpty()) {
        ...
        try {
            ...
            IdentityNumber identityNumber = ParserUtil.parseIdentityNumber(argMultimap.getValue(PREFIX_IDENTITY_NUMBER).get());
