...
public class AddPropertyToSellParser implements Parser<AddPropertyToSellCommand> {
    ...
    public AddPropertyToSellCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_HOUSING_TYPE, PREFIX_SELLING_PRICE, PREFIX_POSTAL_CODE, PREFIX_UNIT_NUMBER, PREFIX_TAG);

        if (!arePrefixesPresent(argMultimap, PREFIX_HOUSING_TYPE, PREFIX_SELLING_PRICE, PREFIX_POSTAL_CODE, PREFIX_UNIT_NUMBER)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddPropertyToSellCommand.MESSAGE_USAGE));
        }
        // Create a new Property object here and pass it to AddPropertyToSellCommand(Property property);
        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_HOUSING_TYPE, PREFIX_SELLING_PRICE, PREFIX_POSTAL_CODE, PREFIX_UNIT_NUMBER);
