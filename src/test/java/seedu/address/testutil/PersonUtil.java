...
public class PersonUtil {
    ...
    public static String getPersonDetails(Person person) {
        ...
        person.getIssues().stream().forEach(
            s -> sb.append(PREFIX_ISSUE + s.issueName + " ")
        );
        return sb.toString();
    }
    ...
    public static String getEditPersonDescriptorDetails(EditPersonDescriptor descriptor) {
        ...
        if (descriptor.getIssues().isPresent()) {
            Set<Issue> issues = descriptor.getIssues().get();
            if (issues.isEmpty()) {
                sb.append(PREFIX_ISSUE);
            } else {
                issues.forEach(s -> sb.append(PREFIX_ISSUE).append(s.issueName).append(" "));
            }
        }
        return sb.toString();
    }
