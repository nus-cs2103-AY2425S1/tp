...
public class UntagCommandTest {
    ...
    @Test
    public void execute_oneTagRemovalUnfilteredList_success() {
    ...
    public void execute_removeAllTags_success() {
        ...
        // List all tags that existed before removal
        String removedTags = personToEdit.getTags().stream()
                .map(Tag::toString)
                .collect(Collectors.joining(", "));

        String expectedMessage = String.format(UntagCommand.MESSAGE_UNTAG_SUCCESS + ". Tags removed: [%2$s]",
                personToEdit.getName(), removedTags);
    ...
    public void execute_emptyTagSet_throwsCommandException() {
        Index index = INDEX_FIRST_PERSON;
        Set<Tag> emptyTagSet = SampleDataUtil.getTagSet(); // Empty set
        UntagCommand untagCommand = new UntagCommand(index, emptyTagSet);

        assertCommandFailure(untagCommand, model, "Error: No tags to remove from this person.");
    }
