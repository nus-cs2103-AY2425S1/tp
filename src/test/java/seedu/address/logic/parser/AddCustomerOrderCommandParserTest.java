...
public class AddCustomerOrderCommandParserTest {
    private AddCustomerOrderCommandParser parser = new AddCustomerOrderCommandParser();

    @Test
    public void parse_allFieldsPresent_success() {
        // Base case
        ArrayList<Integer> idList = new ArrayList<>(Arrays.asList(1, 2));
        AddCustomerOrderCommand expectedCommand = new AddCustomerOrderCommand(
        ...
        assertParseSuccess(parser, NAME_DESC_BOB + PHONE_DESC_BOB
                ...
                new AddCustomerOrderCommand(new Name(VALID_NAME_BOB), new Phone(VALID_PHONE_BOB),
                        new ArrayList<>(Arrays.asList(1, 2, 3)), new Remark(VALID_REMARK_BOB)));
    }

    @Test
    public void parse_optionalFieldsMissing_success() {
        // missing name prefix - defaults to "Guest Customer"
        ArrayList<Integer> idList = new ArrayList<>(Arrays.asList(1, 2));
        AddCustomerOrderCommand expectedCommand = new AddCustomerOrderCommand(
                new Name("Guest Customer"), new Phone(VALID_PHONE_BOB), idList, new Remark(""));
        assertParseSuccess(parser, PHONE_DESC_BOB + " " + PREFIX_ORDER + "1 2", expectedCommand);

        // missing remark prefix - defaults to empty string
        expectedCommand = new AddCustomerOrderCommand(
    ...
    public void parse_compulsoryFieldMissing_failure() {
        String expectedMessage = String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCustomerOrderCommand.MESSAGE_USAGE);
