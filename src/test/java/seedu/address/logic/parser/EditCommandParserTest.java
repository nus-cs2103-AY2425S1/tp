...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_TELEHANDLE_DESC,
                TelegramHandle.MESSAGE_CONSTRAINTS); // invalid telegram handle
        assertParseFailure(parser, "1" + INVALID_CONTACTTYPE_DESC,
                ContactType.MESSAGE_CONSTRAINTS); // invalid contact type
        assertParseFailure(parser, "1" + INVALID_TAG_DESC, Tag.MESSAGE_CONSTRAINTS); // invalid tag
        assertParseFailure(parser, "1" + INVALID_MODNAME_DESC,
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_HUSBAND
                + EMAIL_DESC_AMY + NAME_DESC_AMY + TELEHANDLE_DESC_AMY + CONTACTTYPE_DESC_AMY + MODNAME_DESC_AMY
                + REMARK_DESC_AMY + TAG_DESC_FRIEND;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withContactType(VALID_CONTACTTYPE_AMY)
                .withTelegramHandle(VALID_TELEHANDLE_AMY).withModuleName(VALID_MODNAME_AMY)
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        descriptor = new EditPersonDescriptorBuilder().withTelegramHandle(VALID_TELEHANDLE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // contact type
        userInput = targetIndex.getOneBased() + CONTACTTYPE_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withContactType(VALID_CONTACTTYPE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // module name
        userInput = targetIndex.getOneBased() + MODNAME_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withModuleName(VALID_MODNAME_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
